...
# hello-app-1의 version label 내용 수정
kubectl label pod hello-app-1 version=v2 --overwrite

# 특정 label key에 해당하는 value만 출력
kubectl get pod/my-pod --label-columns app, env
kubectl get pod/my-pod -L app, env  # 단축어 -L = --label-columns

# 특정 label 삭제
kubectl label pod/my-pod app-   # app label 삭제
# 삭제 된 내역 확인을 위한 label조회
